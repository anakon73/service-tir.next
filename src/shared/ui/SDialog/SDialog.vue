...
<template>
  <Dialog as="div" :open="open" class="relative z-20">
    ...
    <div
      class="
      fixed inset-0 z-50 flex h-full items-center
      justify-center overflow-y-auto p-5 text-center
      "
    >
      <DialogPanel
        ref="panel"
        :class="
          cn(
            `w-full overflow-hidden rounded-[30px] bg-white relative
             py-10 text-left align-middle transition-all`,
            size === 'md' && 'max-w-md',
            size === 'lg' && 'max-w-[600px]',
            props.class,
          )
